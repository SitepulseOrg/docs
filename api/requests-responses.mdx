...
### Problem event status (internal)

| Value | Description |
|-------|-------------|
| `open` | Problem is currently active (notifications may be firing) |
| `resolved` | Problem has recovered |

<Note>
  These are internal states used for notification deduplication. The API only supports read access.
</Note>
