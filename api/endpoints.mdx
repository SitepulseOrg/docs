...
Read team monitoring defaults: enabled tools, cadences, status-check header, issue settings, quiet time, escalation, cadence presets, and UI helper metadata.
...
- The tool must not already have a `pending` check on the site
- The same tool can only be run again after its minimum on-demand cadence has passed for the site
...
| `This site already has a pending {tool} check.` | A check of that tool is still running on the site |
| `{Tool} checks can only be run every {cadence} for this site.` | The tool was run too recently; wait for the minimum on-demand cadence |
