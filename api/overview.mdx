...
<CardGroup cols={2}>
  <Card title="v1 collection" icon="download" href="/postman/SitePulse-v1.postman_collection.json">
...
<AccordionGroup>
  <Accordion title="Import and configure">
    1. In Postman, click **Import** and upload [SitePulse-v1.postman_collection.json](/postman/SitePulse-v1.postman_collection.json) and [SitePulse.postman_environment.json](/postman/SitePulse.postman_environment.json).
    2. Select the **SitePulse Production** environment.
    3. Set `auth_token` to your [personal access token](/guide/account-settings#api-tokens). Leave `base_url` as `https://app.sitepulse.dev` unless you use a self-hosted instance.
  ...
  <Accordion title="Full export (internal routes)">
    The generator also produces a [full collection](/postman/SitePulse.postman_collection.json) that includes non-versioned `/api/` routes used by the web dashboard. Those routes are **not** part of the public v1 contract—use the **v1** folder only for integrations.
  </Accordion>
</AccordionGroup>
...
<Steps>
  ...
  <Step title="Create a site and run a check">
    ...
    const siteResponse = await fetch(
      `https://app.sitepulse.dev/api/v1/teams/${teamId}/sites`,
      {
        ...
        body: JSON.stringify({
          ...
          name: 'Example',
          settings: { enabled_checks: ['status'] },
        }),
      }
    );
    ...
    // Create a site
    siteBody := strings.NewReader(`{"url":"https://example.com","name":"Example","settings":{"enabled_checks":["status"]}}`)
