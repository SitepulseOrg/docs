  ...
  <Step title="Create a site and run a check">
    ...
    const checkResponse = await fetch(
      `https://app.sitepulse.dev/api/v1/sites/${site.data.uuid}/checks`,
      {
        method: 'POST',
        headers,
        body: JSON.stringify({ tools: ['status'], automatic: false }),
      }
    );
    ...
    check = requests.post(
        ...
        headers=headers,
        json={'tools': ['status'], 'automatic': False},
    ).json()
    ...
    // Run a status check (returns 202 with pending check records)
    checkBody := strings.NewReader(`{"tools":["status"],"automatic":false}`)
    ...
    check_request.body = { tools: ['status'], automatic: false }.to_json
    ...
    curl_setopt_array($ch, [
        CURLOPT_POST => true,
        CURLOPT_HTTPHEADER => array_map(fn ($k, $v) => "$k: $v", array_keys($headers), $headers),
        CURLOPT_POSTFIELDS => json_encode(['tools' => ['status'], 'automatic' => false]),
        CURLOPT_RETURNTRANSFER => true,
    ]);
