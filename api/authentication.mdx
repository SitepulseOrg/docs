...
For AI clients, use **Settings -> MCP** instead. MCP supports browser OAuth for compatible clients and a scoped token fallback for clients that need bearer-token auth. See [MCP integrations](/api/mcp).
