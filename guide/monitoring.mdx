...
Team monitoring settings control how often SitePulse runs automatic checks and when monitoring problems open issues. Admins and owners can edit these settings; members can view them.
...
Cadence presets range from every minute to every week. Your [plan](/guide/billing#plans) sets the **fastest** cadence allowed: Starter 30 minutes, Growth 5 minutes, Agency 1 minute.

Broken-link and performance checks have an additional 12-hour minimum cadence floor because they are more expensive to run. SitePulse disables cadence options that are faster than your plan or the tool-specific floor.

When a site's enabled checks or cadence overrides change, SitePulse resyncs that site's automatic schedules.
...
A default HTTP header sent with every status check across the team. Use this when monitored sites require authentication or a custom header to respond correctly.
...
Issue opening can be enabled or disabled per check tool. When disabled, automatic checks for that tool still run and save results, but they do not open or resolve issues.
