...
In the dashboard, on-demand check access depends on your role, your plan's tools, and how many on-demand checks your team has left this month. You can run a dashboard on-demand check even when that tool's automatic schedule is off for the site. The v1 API is stricter and only accepts tools enabled on the site. The same tool can't be run on demand again until any pending on-demand check finishes and its minimum on-demand cadence has passed for that site. Automatic checks do not block on-demand runs.
