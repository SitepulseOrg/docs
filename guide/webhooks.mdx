...
<Steps>
  ...
  <Step title="Choose events">
    Select one or both:
    - **Issue detected** — fires when a new issue opens or a resolved issue reopens
...
| **Events** | All | `open` (Issue detected) and/or `resolve` (Issue resolved) |
...
2. The issue transition matches a subscribed **event** (`open` or `resolve`)
...
<Note>
  Acknowledging or refreshing an issue does not send webhook alerts. Only **open** (including reopen) and **resolve** transitions trigger delivery.
</Note>
...
|--------|---------------|----------------|-------|
| `X-SitePulse-Event` | `notification.test` | `issue.opened` or `issue.recovered` | Event name |
...
The `transition` field in the payload reflects the internal transition (`open` or `resolve`), not `reopen`.
...
|-------|------|-------------|
| `event` | string | `issue.opened` or `issue.recovered` |
| `transition` | string | `open` or `resolve` |
