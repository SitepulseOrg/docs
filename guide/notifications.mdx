...
SitePulse notifies your team when automatic monitoring opens, escalates, or resolves issues, and when manual checks complete or fail in the background.
...
HTTP-backed destinations can be tested from the notification configuration. Test delivery verifies connectivity and formatting, but it sends a test payload, not a real issue payload.
